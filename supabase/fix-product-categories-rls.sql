...
begin
  ...
  loop
    ...
    execute format(
      ...
      left('pc_' || pol.policyname, 63),   -- حد اسم الكائن في Postgres
    ...
    raise notice 'أُنشئت السياسة: %', left('pc_' || pol.policyname, 63);
  end loop;
...
--
--    ملاحظة: نقرأ العمود p.prosrc مباشرةً ولا نستخدم pg_get_functiondef()،
--    لأن تلك الدالة ترمي خطأ 42809 عند أول دالة تجميع تصادفها
--    ("array_agg" is an aggregate function)، ومُخطِّط Postgres قد ينفّذها
--    قبل شرط nspname = 'public' فتطال دوال pg_catalog وتُلغي المعاملة كلها.
-- ─────────────────────────────────────────────────────────────────────────────
select
...
where n.nspname = 'public'
  and p.prokind in ('f', 'p')          -- دوال وإجراءات فقط، بلا aggregates
  and p.prosrc ~* '\muser_id\M'
  and p.prosrc ~* '\mtenants\M';
