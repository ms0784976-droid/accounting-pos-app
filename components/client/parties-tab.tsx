    ...
    <div className="space-y-5">
      ...
      <ConfirmDialog
        ...
        message={
          deleting
            ? `حذف "${deleting.name}"؟\n\n` +
              `إذا لم تكن عليه أي فاتورة أو سند سارٍ ولا رصيد — يُحذف نهائياً، ` +
              `حتى لو كانت كل فواتيره ملغاة.\n\n` +
              `أما إن كانت عليه حركة سارية فيُنقل إلى الأرشيف بدل الحذف — يختفي من ` +
              `القوائم والفواتير الجديدة، ويبقى في السجل حتى لا تنكسر كشوف الحسابات. ` +
              `وسنخبرك بالضبط أيّ الحالتين حصلت ولماذا.`
            : ""
        }
        confirmLabel="حذف"
        onConfirm={async () => {
          ...
          // الرسالة صارت تشرح ما حدث ولماذا، بدل "تم التعطيل" الغامضة
          notify(
            res.deactivated
              ? res.reason
              : `تم حذف "${deleting.name}" نهائياً — لم تكن عليه أي حركة سارية.`
          )
          parties.reload()
        }}
      />
